...
object PerfectNumbers {
  def classify(int: Int): Either[String, NumberType] = {
    if (int <= 0) Left("Classification is only possible for natural numbers.")
      ...
      val aliquotSum: Int = calcAliquotSum(int)
      if (aliquotSum == int) Right(NumberType.Perfect)
      else if (aliquotSum > int) Right(NumberType.Abundant)
      else Right(NumberType.Deficient)
    }
  }

  def calcAliquotSum(int: Int): Int = int.factors.sum
